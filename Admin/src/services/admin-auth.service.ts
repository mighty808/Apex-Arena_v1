...
export const adminAuthService = {
  ...
  /**
   * Validate admin token using admin-authenticated route.
   * /admin/token/validate is internal-only in backend.
   */
  async validateToken(accessToken: string): Promise<{ user?: AdminUser }> {
    const response = await apiGet(AUTH_ENDPOINTS.ADMIN_SESSIONS, {
    ...
    return rawUser ? { user: mapAdminUser(rawUser) } : {};
  },
