import { getAdminAccessToken, getAdminRefreshToken, saveAdminTokens } from './auth.utils';
...
export const refreshAccessToken = async (): Promise<boolean> => {
  try {
    ...
    const response = await fetch(AUTH_ENDPOINTS.ADMIN_TOKEN_REFRESH, {
      method: 'POST',
    ...
    const nextAccessToken = String(payload.access_token ?? payload.accessToken ?? '');
    const nextRefreshToken = (payload.refresh_token ?? payload.refreshToken ?? refreshToken) as string | undefined;

    if (!nextAccessToken) {
      console.error('[API] Token refresh response missing access token');
      return false;
    }
